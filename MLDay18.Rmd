...
* It would also be a plus to know:
    ...
    - What is the shape of the data---how does it cluster?
...
# Control for maximally complex tree
...
```{r rpart-vi}
cart2$variable.importance[1L:3L]
```
...
```
<br>
.center[**Source:** The Elements of Statisticl Learning, Second Edition]
...
* Thus, roughly $e^{-1}$ of the original obervations are not used in the construction of a particular tree
...
* **Traditional approach:** Average variable importance scores (i.e., toal goodness of split) over all trees 
...
* **Novel approach:** To estimate the importance of the $k$-th variable:
  ...
  4. The difference between the two OOB performance measures measrues the strength of the strucural importance of $x_k$
  
* FUndamentally different as these importanc measures are **NOT** based on data seen by the individual trees!
...
* The pairwise proximitiescan be used to construct a useful distance matrix
...
* Random forest offers a novel approach to imputing missing values (i.e., `NA`s)
    ...
    1. Start with cheap impuation (e.g., median for continuous features)
    
    2. Run a random forest and obtain the proximity matrix
    
    3. Imputed values are updated using:
    
        - The weighted average of the non-missing obervations, where the weights are the proximities (**continuous**)
...
* Resulting OOB estimate of error tends to be verly ooptimistic
